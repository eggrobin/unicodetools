...
for graph_name in ("cherry", "triangle", "path"):
  print("===", graph_name, "===")
  symbols : list[str] = []
  with open(graph_name+"BreakSymbols.txt") as f:
  ...
  with open(graph_name+"BreakTransitions.txt") as f:
    for line in f.readlines():
      line = line.split("#")[0].strip()
      if not line:
        continue
  ...
  with open(graph_name+"BreakStates.txt") as f:
    for line in f.readlines():
      line = line.split("#")[0].strip()
      if not line:
        continue
  ...
  states_by_type : dict[tuple[str, int|None], set[str]] = defaultdict(set)
  for state in states:
    states_by_type[accepting[state] if accepting[state] in ("Yes", "No") else
                   "lookahead #" + str(lookahead_colours[accepting[state]]),
  ...
  print("from", sum(len(g) for g in Π))
